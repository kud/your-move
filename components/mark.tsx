/*
 * The mark, inlined rather than fetched.
 *
 * 700 bytes of path data is cheaper than the request that would go and get it,
 * and an asset the service worker does not precache renders as a broken image
 * on the offline page — which is the one screen where a broken logo would be
 * the most visible thing on it.
 *
 * `aria-hidden` because the `<h1>` beside it already says "Your Move": a mark
 * that announces the name a second time is noise in a screen reader, not
 * redundancy that helps.
 *
 * Before anyone recolours this: rose is `--color-accent`, and in this app that
 * value means exactly one thing — "this needs you". It is the count in the
 * header, the focus ring, the stripe on a card. A permanent rose object in the
 * same header as the live count spends that signal, which is why Iris signed
 * the placement on the ivory `mark-mono` and not this one. If the two ever read
 * as fighting, the fix is to make both paths `#e9ebee`, not to shrink the mark.
 */
export const Mark = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 400 352" aria-hidden className={className}>
    <g transform="translate(37 38)">
      <path
        fill="#e0707c"
        d="M 0 37 C 0 8 28 -9 53 6 L 221 104 C 230 109 234 116 234 125 L 234 190 C 234 199 226 204 218 199 L 99 129 C 88 122 77 129 77 141 L 77 263 C 77 273 70 279 61 274 L 15 247 C 5 241 0 231 0 220 Z"
      />
      <path
        fill="#e9ebee"
        d="M 180 61 L 272 7 C 296 -7 326 9 326 35 L 326 220 C 326 232 321 242 311 248 L 264 274 C 256 279 247 274 247 263 L 247 122 C 247 109 242 101 231 94 L 180 65 Q 177 63 180 61 Z M 90 151 Q 90 142 97 147 L 166 187 Q 175 192 166 198 L 97 241 Q 90 245 90 237 Z"
      />
    </g>
  </svg>
)
